...
const getTimeBasedGreeting = () =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (hour < 12) return config.morningMessage;
    if (hour < 17) return config.afternoonMessage;
        ...
        __awaiter(void 0, void 0, void 0, function* () {
          ...
          try {
            ...
            for (const product of products) {
              try {
                const productKeywords = JSON.parse(product.keywords || "[]").map(
                  helperFunctions_1.cleanText
                );
                const isMatch = productKeywords.some((productKw) => {
                  const fullSimilarity = string_similarity_1.default.compareTwoStrings(
                    cleanedQuery,
                    productKw
                  );
                  if (fullSimilarity >= SIMILARITY_THRESHOLD) return true;
                  return keywords.some((kw) => {
                    const wordSimilarity = string_similarity_1.default.compareTwoStrings(
                      kw,
                      productKw
                    );
                    return wordSimilarity >= SIMILARITY_THRESHOLD || productKw.includes(kw);
                  });
                });
                ...
                if (product.deliveryText) {
                  yield whatsAppClient === null || whatsAppClient === void 0
                    ? void 0
                    : whatsAppClient.sendMessage(message.from, product.deliveryText);
                }
              } catch (productErr) {
